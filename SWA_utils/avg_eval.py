...
tf.set_random_seed(2)  # for reproducibility
if __name__ == "__main__":
  ...
  flags.DEFINE_boolean("swa", False, "")
  flags.DEFINE_integer("swa_start", -1, "")
...
def get_checkpoints_start(start):
  index_files = file_io.get_matching_files(os.path.join(FLAGS.train_dir, 'model.ckpt-*.index'))
...
def evaluation_loop(video_id_batch, prediction_batch, label_batch, loss,
  ...
  with tf.Session() as sess:
    if FLAGS.swa:
      ckpts = get_checkpoints_start(FLAGS.swa_start)
      swa_var = {}
      for v in tf.trainable_variables():
        swa_var[v.op.name] = np.zeros(v.get_shape().as_list(), dtype=np.float32)

      swa_n = 0.
      for ckpt in ckpts:
        print 'SWA restoring from %s' % ckpt
        saver.restore(sess, ckpt)
        for v in tf.trainable_variables():
          v_val = v.eval(sess)
          swa_var[v.op.name] = (swa_var[v.op.name]*swa_n+v_val) / (swa_n+1)
        swa_n += 1
        global_step_val = os.path.basename(ckpt).split("-")[-1]

      for v in tf.trainable_variables():
        v.load(swa_var[v.op.name], sess)
