...
class BiGruModel(models.BaseModel):
  ...
  def create_model(self, model_input, vocab_size, num_frames, is_training=True, **unused_params):
    ...
    with tf.variable_scope('fw_cell'):
      fw_cell = tf.contrib.rnn.GRUCell(gru_size)
    with tf.variable_scope('bw_cell'):
      bw_cell = tf.contrib.rnn.GRUCell(gru_size)

    loss = 0.0
    with tf.variable_scope("RNN"):
      outputs, state = tf.nn.bidirectional_dynamic_rnn(fw_cell, bw_cell, model_input, sequence_length=num_frames,
                                         dtype=tf.float32)

    act = tf.concat(state, -1)
    aggregated_model = getattr(video_level_models,
                               'willow_MoeModel_moe4')
    return aggregated_model().create_model(
        model_input=act,
...
class GatedDbofModelLF_1024k(models.BaseModel):
  """Creates a Gated Deep Bag of Frames model.
  The model projects the features for each frame into a higher dimensional
  'clustering' space, pools across frames in that space, and then
  uses a configurable video-level model to classify the now aggregated features.
...
class NetFVModelLF_16k(models.BaseModel):
  """Creates a NetFV based model.
     It emulates a Gaussian Mixture Fisher Vector pooling operations
  Args:
    model_input: A 'batch_size' x 'max_frames' x 'num_features' matrix of
...
class AddNetVLADModelLF(models.BaseModel):
  """Creates a NetVLAD based model.
  Args:
    model_input: A 'batch_size' x 'max_frames' x 'num_features' matrix of
                 input features.
