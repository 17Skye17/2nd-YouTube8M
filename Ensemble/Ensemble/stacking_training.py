...
if __name__ == "__main__":
  ...
  flags.DEFINE_string("train_dir", "/home/skye/Ensemble_log",
...
class Trainer(object):
  ...
  def __init__(self, cluster, task, train_dir, subdirs, model, reader, graph, model_exporter,
    ...
    if FLAGS.start_new_model == False:
        ...
        self.train_dir = train_dir + '_' + datetime.strftime(datetime.now(), "%m%d%H%M")
...
#                           task_as_string(self.task))
  ...
  def run(self, start_new_model=False):
    ...
    sv = tf.train.Supervisor(
        ...
        saver=None if FLAGS.start_new_model == True else saver)
...
def main(unused_argv):
  ...
  for subdir in subdirs:
    ...
    if save_flag:
      ...
      shutil.copyfile(model_flags_path,os.path.join(FLAGS.train_dir,"model_flags.json"))
      save_flag = False
  ...
  if not cluster or task.type == "master" or task.type == "worker":
    ...
    with tf.Graph().as_default() as graph:
        Trainer(cluster, task, FLAGS.train_dir, subdirs, models, reader, graph,        model_exporter,FLAGS.log_device_placement, FLAGS.max_steps,
