...
if __name__ == "__main__":
  ...
  # Training flags.
  flags.DEFINE_integer("save_model_steps",10,"the internal for saving models.")
...
def combine_models(models,model_input,num_frames,reader,labels_batch,is_training):
    ...
    print "model_nums = %d"%model_nums
    prob_ratio = tf.get_variable("prob_ratio",
              [model_nums],
              initializer = tf.constant_initializer(1 / float(model_nums)))
    prob_ratio = prob_ratio / tf.reduce_sum(prob_ratio)
    predictions = 0
...
def build_graph(reader,
    ...
    with tf.device(device_string % i):
      with (tf.variable_scope(("tower"), reuse=True if i > 0 else None)):
        with (slim.arg_scope([slim.model_variable, slim.variable], device="/cpu:0" if num_gpus != 1 else "/gpu:0")):
          ...
          result = combine_models(models,tower_inputs[i],tower_num_frames[i],reader,labels_batch=tower_labels[i],is_training=False)
  ...
  tune_merged_gradients = [x for x in merged_gradients if 'prob_ratio' in x[1].name]
  print('tune_merged_gradients :', tune_merged_gradients)

  train_op = optimizer.apply_gradients(tune_merged_gradients, global_step=global_step)
...
class Trainer(object):
  ...
  def __init__(self, cluster, task, train_dir, subdirs, model, reader, graph,  model_exporter,
...
#
    ...
    with sv.managed_session(target, config=self.config) as sess:
      try:
        ...
        for i in range(len(self.model)):
          load_vars(sess,self.subdirs[i],"model"+str(i))
        ...
        while (not sv.should_stop()) and (not self.max_steps_reached):
          ...
          if (global_step_val -1)%FLAGS.save_model_steps==0:
              saver.save(sess,os.path.join(self.train_dir, 'model.ckpt'),global_step_val)
  ...
  def build_model(self, model, reader):
    ...
    return saver
    #return tf.train.Saver(var_list=tf.global_variables(),max_to_keep=10,
...
def load_vars(sess,train_dir,scope):
    model_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope="tower/"+scope)
    ...
    print os.path.join(FLAGS.Ensemble_Models,train_dir,"model.ckpt-1")
...
def main(unused_argv):
  ...
  for subdir in subdirs:
    ...
    if save_flag:
      ...
      train_dir = train_dir + datetime.strftime(datetime.now(),"%m%d%H%M")
      if os.path.exists(train_dir) == False:
        os.mkdir(train_dir)
      shutil.copyfile(model_flags_path,os.path.join(train_dir,"model_flags.json"))
      save_flag = False
  ...
  if not cluster or task.type == "master" or task.type == "worker":
   ...
   #            [frame_level_models, video_level_models])()
    ...
    with tf.Graph().as_default() as graph:
        Trainer(cluster, task, train_dir, subdirs, models, reader, graph, model_exporter,FLAGS.log_device_placement, FLAGS.max_steps,
