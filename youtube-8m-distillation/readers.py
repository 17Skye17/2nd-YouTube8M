...
class YT8MDDistillFrameFeatureReader(BaseReader):
  """Reads TFRecords of SequenceExamples.
  ...
  def prepare_serialized_examples(self, serialized_example,
    ...
    contexts, features = tf.parse_single_sequence_example(
        ...
        context_features={"id": tf.FixedLenFeature(
            [], tf.string),
                          "ensemble_labels": tf.FixedLenFeature([], tf.string),
                          "labels": tf.VarLenFeature(tf.int64)},
        sequence_features={
            feature_name : tf.FixedLenSequenceFeature([], dtype=tf.string)
            for feature_name in self.feature_names
    ...
    ensemble_labels = tf.decode_raw(contexts["ensemble_labels"], tf.float32)
    ensemble_labels = tf.reshape(ensemble_labels, [3862])
    ...
    batch_labels = tf.expand_dims(labels, 0)
    batch_frames = tf.expand_dims(num_frames, 0)
    batch_predict_labels = tf.expand_dims(ensemble_labels, 0)

    return batch_video_ids, batch_video_matrix, batch_labels, batch_frames, batch_predict_labels