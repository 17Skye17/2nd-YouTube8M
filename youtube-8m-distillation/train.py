...
if __name__ == "__main__":
  ...
  flags.DEFINE_float(
      "distill_prob", 1.0,
      "How much weight for distillation training.")
...
def build_graph(reader,
  ...
  unused_video_id, model_input_raw, labels_batch, num_frames, predict_label = (
  ...
  tower_predict_label = tf.split(predict_label, num_towers)
  tower_gradients = []
  tower_predictions = []
  tower_label_losses = []
  tower_reg_losses = []
  p = FLAGS.distill_prob
      ...
      with (tf.variable_scope(("tower"), reuse=True if i > 0 else None)):
        with (slim.arg_scope([slim.model_variable, slim.variable], device="/cpu:0" if num_gpus != 1 else "/gpu:0")):
          ...
          if "loss" in result.keys():
            ...
            if p > 1.0: # CE with ensemble label
              label_loss = label_loss_fn.calculate_loss(predictions, tower_predict_label[i]/5.0)
            elif p < 0.0:  # CE with gt label
              label_loss = label_loss_fn.calculate_loss(predictions, tower_labels[i])
            else:
              label_loss = (1.0-p)*label_loss_fn.calculate_loss(predictions, tower_labels[i]) + p*label_loss_fn.calculate_loss(predictions, tower_predict_label[i]/5.0)
  ...
  def run(self, start_new_model=False):
    ...
    with tf.Graph().as_default() as graph:
      ...
      #     saver = self.build_model(self.model, self.reader)
        saver = self.build_model(self.model, self.reader)
        tmp_var = [var for var in tf.global_variables() if 'Adam' not in var.name and 'beta1_power' not in var.name and 'beta2_power' not in var.name]
        print('tmp_var')
        print(tmp_var)
        tmp_saver = tf.train.Saver(var_list=tmp_var, reshape=True)
    ...
    pretrain_dir = tf.train.latest_checkpoint(self.train_dir, 'checkpoint_bat')
    def load_pretrain(sess):
        tmp_saver.restore(sess, pretrain_dir)
...
def get_reader():
  ...
  if FLAGS.frame_features:
    reader = readers.YT8MDDistillFrameFeatureReader(
