    ...
    with tf.device(device_string % i):
      with (tf.variable_scope(("tower"), reuse=True if i > 0 else None)):
        with (slim.arg_scope([slim.model_variable, slim.variable], device="/cpu:0" if num_gpus != 1 else "/gpu:0")):
          ...
          predictions1 = result[0]["predictions"]
          predictions2 = result[1]["predictions"]
          tower_predictions.append(predictions1+predictions2)

          label_loss = label_loss_fn.calculate_loss(predictions1, tower_labels[i]) + label_loss_fn.calculate_loss(predictions2, tower_labels[i])
          ...
          if "update_ops" in result[0].keys():
            update_ops += result["update_ops"]
          if "update_ops" in result[1].keys():
            update_ops += result["update_ops"]
          if update_ops:
