...
if __name__ == "__main__":
  ...
  flags.DEFINE_integer("seed", 2018,
                       "seed.")
  flags.DEFINE_string("checkpoint_prefix", None,
                      "checkpoint prefix for average. seperate by comma. e.g.: model.ckpt-10000,model.ckpt-20000")
...
def averaging_checkpoint(checkpoint_prefix):
  checkpoint_file_list = checkpoint_prefix.split(',')

  checkpoint_dict = {}

  for checkpoint_i, checkpoint_file in enumerate(checkpoint_file_list):
    reader = pywrap_tensorflow.NewCheckpointReader(os.path.join(FLAGS.train_dir,checkpoint_file))
    var_to_shape_map = reader.get_variable_to_shape_map()

    for key in var_to_shape_map:
      checkpoint_array = reader.get_tensor(key)
      # If the var_name not exist in dict
      if key not in checkpoint_dict.keys():
        checkpoint_dict[key] = checkpoint_array
      # Else average the paramters
      else:
        checkpoint_dict[key] += checkpoint_array

  # average parameters
  for key in checkpoint_dict.keys():
    checkpoint_dict[key] /= len(checkpoint_file_list)
...
def evaluation_loop(video_id_batch, prediction_batch, label_batch, loss,
  ...
  with tf.Session(config=config) as sess:
    print('number of trainable variables: %d' % len(tf.trainable_variables()))
    print('number of global variables: %d' % len(tf.global_variables()))
    if FLAGS.checkpoint_prefix is None:
      raise IOError(("checkpoint_prefix %s is wrong.") % FLAGS.checkpoint_prefix)
    weights_avg_array = averaging_checkpoint(FLAGS.checkpoint_prefix)

    print('number of parameters after averaging: %d' % len(weights_avg_array.keys()))
